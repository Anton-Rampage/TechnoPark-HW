...
thread_local struct Ordinator {
    static constexpr size_t STACK_SIZE = 1ul << 16;
...
struct Routine {
    ...
    Routine(const RoutineFunction& f) : func(f), stack(std::make_unique<uint8_t[]>(Ordinator::STACK_SIZE)) {
...
void entry() {
    ...
    if (routine.func) {
        try {
            routine.func();
        } catch (...) { routine.exception = std::current_exception(); }
    }
