...
EpollServer::EpollServer(uint32_t port, Handlers& handlers) {
    _epoll_fd = Descriptor(epoll_create(1));
    if (_epoll_fd < 0) {
...
void EpollServer::open(uint32_t port, Handlers& handlers) {
    Descriptor serv(socket(PF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0));
    if (serv < 0) {
...
void EpollServer::epoll_action(int fd, uint32_t events, int action) {
    ...
    if (epoll_ctl(_epoll_fd.get(), action, fd, &event) < 0) {
        throw BaseException("epoll ctl add failed");
    }
}
...
void EpollServer::accept() {
    ...
    Descriptor fd;
    while (true) {
        fd = ::accept4(_server_fd.get(), reinterpret_cast<sockaddr*>(&client_addr), &addr_size, SOCK_NONBLOCK);
        if (fd < 0) {
            if (errno == EINTR) {
                continue;
        ...
        int buf_fd = fd.get();
        EpollConnection connection(std::move(fd));
        _connections.insert(std::pair<int, EpollConnection>(buf_fd, std::move(connection)));
